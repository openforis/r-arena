...
arenaAnalytics <- function( arena.analyze ) {
  ...
  # arena.analyze   <- list(entity = "tree", dimensions = c("cluster_stratum", "tree_sampletree", "plot_fra", "plot_landuse"), filter = "plot_landuse == '1'", reportMethod = '2')
 # arena.analyze   <- list(entity = "tree", dimensions = c("cluster_stratum", "tree_sampletree", "plot_fra", "tree_species"), filter = "", reportMethod = '2')
  # ***call as follows:
  # arena_process_response <- arenaAnalytics( arena.analyze )
  # print( arena_process_response) 
  
  # tree$tree_species[ tree$tree_no==1 ] <- "ALB/GLA"
  # tree$tree_species[ tree$tree_no==2 ] <- "AFZ/QUA"
  # 
  #*********************************************************************'  
  #
  ...
  if (!("filter" %in% names(arena.analyze) )) arena.analyze$filter <- ""
  if (is.na(arena.analyze$filter))            arena.analyze$filter <- ""
  if (is.null(arena.analyze$filter))          arena.analyze$filter <- ""
  if (is.null(arena.analyze$entity) | arena.analyze$entity =="" | is.na(arena.analyze$entity) | length(arena.analyze$entity)==0 ){
    return( "No entity to report" )
  }
  if (is.null(arena.analyze$dimensions) | is.na(arena.analyze$entity) | length(arena.analyze$entity)==0 ){
    return( "No dimension to report" )
  } else {
...
#        rm(boolean_list)
  ...
  # do analysis based on reportMethod
  # '1' = Combination of dimensions (default)
  # '2' = Dimensions separately
  
  if ( arena.analyze$reportMethod == '1' ) arena.reportingLoops = 1
  if ( arena.analyze$reportMethod == '2' ) {
    arena.reportingLoops           <- length(arena.analyze$dimensions)
    arena.analyze$dimensions_input <- arena.analyze$dimensions
  }  
  
  # get all files in the main output folder
  f <- list.files(user_file_path, full.names = T, recursive = FALSE)
  ...
  for (rep_loop in (1:arena.reportingLoops)) {
    if (arena.analyze$reportMethod == '2') {
      ...
      out_path                 <- paste0(arena.analyze$dimensions, "/")
    ...
    if (exists("result_names_category_2")) rm(result_names_category_2) 
     ...
     cat_names_num <- result_cat[[ arena.analyze$entity ]] %>%
      ...
      # do first filtering, if filter clause exists
      if (arena.analyze$filter != "") {
      ...
      df_analysis_area <- df_analysis_combined                                             %>%
        filter( weight > 0 )                                                               %>%
        group_by(  across( unique( c(cat_names_uuid, arena.analyze$dimensions_baseunit)))) %>%
  ...
  if (arena.analyze$reportMethod == '2') arena.analyze$dimensions <- arena.analyze$dimensions_input 
