...
arenaAnalytics <- function( arena.analyze ) {
  ...
  tryCatch( usePackage('tidyverse'),
            error = function(e){ library('tidyverse')
            })
      ...
      for (i in (1:length(result_entities))) {
        ...
        if (result_entities[[i]] != arena.chainSummary$baseUnit) {
            ...
            if (length(missing_ids) > 0) {
              ...
              result_cat[[i]] <- result_cat[[i]] %>% mutate_if(is.numeric, ~replace_na(., 0))
  ...
  # SAMPLING STRATEGIES AND RELIABILITY
    ...
    result_labels           <- list() 
    result_names_category_1 <- intersect( arena.analyze$dimension, names(get(arena.analyze$entity))) # cat. attributes in input data 
    result_names_category_2 <- setdiff(   arena.analyze$dimension, names(get(arena.analyze$entity))) # cat. attributes not in input data, but as result attributes
    ...
    if (exists("result_names_category_2")) rm(result_names_category_2) 
     ...
     cat_names_num <- result_cat[[ arena.analyze$entity ]] %>%
      ...
      # https://cran.r-project.org/web/packages/srvyr/vignettes/srvyr-vs-survey.html
        ...
        out_file <- list()
        out_file[[1]] <- paste0(user_file_path, arena.analyze$entity, "_out_mean.csv")
        out_file[[2]] <- paste0(user_file_path, arena.analyze$entity, "_out_total.csv")
        ...
        # get all files in the output folder, recursively
        f <- list.files(user_file_path, include.dirs = F, full.names = T, recursive = T)
